...
const StudentRoundLeaderboardPage = ({ roundId: roundIdProp, source = "public" }) => {
  ...
  const notPublished = error?.code === "RESULT_NOT_PUBLISHED" || error?.status === 403 || error?.status === 422;
    ...
    <Space direction="vertical" size={20} style={{ width: "100%" }}>
      <Card
        style={{
          color: "#fff",
          border: 0,
          background: "linear-gradient(135deg, #001529 0%, #003a8c 65%, #1677ff 100%)",
          overflow: "hidden",
        }}
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
          <Space direction="vertical" size={7}>
            ...
            <Title level={1} style={{ color: "#fff", margin: 0 }}>{scoreboard.roundName}</Title>
            <Text style={{ color: "rgba(255,255,255,.78)" }}>
              Bảng điểm chỉ hiển thị sau khi Ban tổ chức hoàn tất và công bố kết quả.
            </Text>
          </Space>
          <Button ghost icon={<ReloadOutlined spin={isLoading} />} onClick={fetchScoreboard}>Làm mới</Button>
        </div>
      </Card>
